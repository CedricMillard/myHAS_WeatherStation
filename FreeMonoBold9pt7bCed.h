#pragma once
#include <Adafruit_GFX.h>

const uint8_t FreeMonoBold9pt7bBitmaps[] PROGMEM = {
  
};

const GFXglyph FreeMonoBold9pt7bGlyphs[] PROGMEM = {
  {     0,   0,   0,  11,    0,    1 },   // 0x20 ' '
  {     5,   3,  11,  11,    4,  -10 },   // 0x21 '!'
  {    10,   7,   5,  11,    2,  -10 },   // 0x22 '"'
  {    15,   8,  12,  11,    1,  -10 },   // 0x23 '#'
  {    27,   7,  14,  11,    2,  -11 },   // 0x24 '$'
  {    40,   7,  11,  11,    2,  -10 },   // 0x25 '%'
  {    50,   8,  10,  11,    1,   -9 },   // 0x26 '&'
  {    60,   3,   5,  11,    4,  -10 },   // 0x27 '''
  {    62,   4,  14,  11,    5,  -10 },   // 0x28 '('
  {    69,   4,  14,  11,    2,  -10 },   // 0x29 ')'
  {    76,   6,   6,  11,    5,  -12 },   // 0x2A '*' but display °
  {    81,   8,   9,  11,    2,   -8 },   // 0x2B '+'
  {    90,   3,   5,  11,    3,   -1 },   // 0x2C ','
  {    92,   9,   2,  11,    1,   -5 },   // 0x2D '-'
  {    95,   2,   2,  11,    4,   -1 },   // 0x2E '.'
  {    96,   7,  15,  11,    2,  -12 },   // 0x2F '/'
  {   110,   7,  12,  11,    2,  -11 },   // 0x30 '0'
  {   121,   7,  11,  11,    2,  -10 },   // 0x31 '1'
  {   131,   8,  12,  11,    1,  -11 },   // 0x32 '2'
  {   143,   8,  12,  11,    2,  -11 },   // 0x33 '3'
  {   155,   7,  10,  11,    2,   -9 },   // 0x34 '4'
  {   164,   9,  11,  11,    1,  -10 },   // 0x35 '5'
  {   177,   8,  12,  11,    2,  -11 },   // 0x36 '6'
  {   189,   8,  11,  11,    1,  -10 },   // 0x37 '7'
  {   200,   7,  12,  11,    2,  -11 },   // 0x38 '8'
  {   211,   8,  12,  11,    2,  -11 },   // 0x39 '9'
  {   223,   2,   8,  11,    4,   -7 },   // 0x3A ':'
  {   225,   3,  11,  11,    3,   -7 },   // 0x3B ';'
  {   230,   9,   8,  11,    1,   -8 },   // 0x3C '<'
  {   239,   9,   6,  11,    1,   -7 },   // 0x3D '='
  {   246,   9,   8,  11,    1,   -8 },   // 0x3E '>'
  {   255,   8,  11,  11,    2,  -10 },   // 0x3F '?'
  {   266,   9,  15,  11,    1,  -11 },   // 0x40 '@'
  {   283,  11,  11,  11,    0,  -10 },   // 0x41 'A'
  {   299,  10,  11,  11,    1,  -10 },   // 0x42 'B'
  {   313,   9,  11,  11,    1,  -10 },   // 0x43 'C'
  {   326,  10,  11,  11,    0,  -10 },   // 0x44 'D'
  {   340,   9,  11,  11,    1,  -10 },   // 0x45 'E'
  {   353,   9,  11,  11,    1,  -10 },   // 0x46 'F'
  {   366,  10,  11,  11,    1,  -10 },   // 0x47 'G'
  {   380,   9,  11,  11,    1,  -10 },   // 0x48 'H'
  {   393,   6,  11,  11,    3,  -10 },   // 0x49 'I'
  {   402,  10,  11,  11,    1,  -10 },   // 0x4A 'J'
  {   416,  10,  11,  11,    1,  -10 },   // 0x4B 'K'
  {   430,   9,  11,  11,    1,  -10 },   // 0x4C 'L'
  {   443,  11,  11,  11,    0,  -10 },   // 0x4D 'M'
  {   459,  11,  11,  11,    0,  -10 },   // 0x4E 'N'
  {   475,  11,  11,  11,    0,  -10 },   // 0x4F 'O'
  {   491,   9,  11,  11,    1,  -10 },   // 0x50 'P'
  {   504,  11,  14,  11,    0,  -10 },   // 0x51 'Q'
  {   524,   9,  11,  11,    1,  -10 },   // 0x52 'R'
  {   537,   9,  11,  11,    1,  -10 },   // 0x53 'S'
  {   550,   9,  11,  11,    1,  -10 },   // 0x54 'T'
  {   563,  11,  11,  11,    0,  -10 },   // 0x55 'U'
  {   579,  11,  11,  11,    0,  -10 },   // 0x56 'V'
  {   595,  10,  11,  11,    0,  -10 },   // 0x57 'W'
  {   609,  10,  11,  11,    0,  -10 },   // 0x58 'X'
  {   623,  10,  11,  11,    0,  -10 },   // 0x59 'Y'
  {   637,   8,  11,  11,    2,  -10 },   // 0x5A 'Z'
  {   648,   4,  14,  11,    5,  -10 },   // 0x5B '['
  {   655,   7,  15,  11,    2,  -12 },   // 0x5C '\'
  {   669,   4,  14,  11,    2,  -10 },   // 0x5D ']'
  {   676,   7,   6,  11,    2,  -11 },   // 0x5E '^'
  {   682,  11,   2,  11,    0,    3 },   // 0x5F '_'
  {   685,   3,   3,  11,    3,  -11 },   // 0x60 '`'
  {   687,   9,   8,  11,    1,   -7 },   // 0x61 'a'
  {   696,  10,  11,  11,    0,  -10 },   // 0x62 'b'
  {   710,   9,   8,  11,    1,   -7 },   // 0x63 'c'
  {   719,  10,  11,  11,    1,  -10 },   // 0x64 'd'
  {   733,   9,   8,  11,    1,   -7 },   // 0x65 'e'
  {   742,   8,  11,  11,    2,  -10 },   // 0x66 'f'
  {   753,   9,  12,  11,    1,   -7 },   // 0x67 'g'
  {   767,   9,  11,  11,    1,  -10 },   // 0x68 'h'
  {   780,   8,  11,  11,    2,  -10 },   // 0x69 'i'
  {   791,   6,  15,  11,    2,  -10 },   // 0x6A 'j'
  {   803,   9,  11,  11,    1,  -10 },   // 0x6B 'k'
  {   816,   8,  11,  11,    2,  -10 },   // 0x6C 'l'
  {   827,  11,   8,  11,    0,   -7 },   // 0x6D 'm'
  {   838,   9,   8,  11,    1,   -7 },   // 0x6E 'n'
  {   847,   9,   8,  11,    1,   -7 },   // 0x6F 'o'
  {   856,  11,  12,  11,    0,   -7 },   // 0x70 'p'
  {   873,  11,  12,  11,    0,   -7 },   // 0x71 'q'
  {   890,   9,   8,  11,    1,   -7 },   // 0x72 'r'
  {   899,   8,   8,  11,    2,   -7 },   // 0x73 's'
  {   907,   8,  11,  11,    1,  -10 },   // 0x74 't'
  {   918,   9,   8,  11,    1,   -7 },   // 0x75 'u'
  {   927,  11,   8,  11,    0,   -7 },   // 0x76 'v'
  {   938,  11,   8,  11,    0,   -7 },   // 0x77 'w'
  {   949,   9,   8,  11,    1,   -7 },   // 0x78 'x'
  {   958,  10,  12,  11,    0,   -7 },   // 0x79 'y'
  {   973,   7,   8,  11,    2,   -7 },   // 0x7A 'z'
  {   980,   4,  14,  11,    3,  -10 },   // 0x7B '{'
  {   987,   2,  14,  11,    5,  -10 },   // 0x7C '|'
  {   991,   4,  14,  11,    4,  -10 },   // 0x7D '}'
  {   998,   9,   4,  11,    1,   -6 }    // 0x7E '~'
};

const GFXfont FreeMonoBold9pt7b PROGMEM = {(uint8_t *)FreeMonoBold9pt7bBitmaps, 
                                           (GFXglyph *)FreeMonoBold9pt7bGlyphs, 0x20, 0x7E,  18};